...
GUI.wnd.name = "New Window"
GUI.wnd.x, GUI.wnd.y, GUI.wnd.w, GUI.wnd.h = 0, 0, 432, 500
...
GUI.new("btn_frm",	"Frame",		1, 0, 56, GUI.wnd.w, 4, true, true)
...
-- :on_open is a hook provided by the Window class. This function will be run
-- every time the window opens.
function GUI.elms.wnd_test:on_open()
    
    -- :adjust_elm places the element's specified x,y coordinates relative to
    -- the Window. i.e. creating an element at 0,0 and adjusting it will put
    -- the element in the Window's top-left corner.
    self:adjust_elm(GUI.elms.btn_close)
    
    -- Buttons look nice when they're centered.
    GUI.elms.btn_close.x, _ = GUI.center(GUI.elms.btn_close, self)    
    
    self:adjust_elm(GUI.elms.lbl_elms)
    self:adjust_elm(GUI.elms.lbl_vals)
...
local function Main()
	...
	if GUI.resized then
		...
		gfx.quit()
		gfx.init(GUI.wnd.name, GUI.wnd.w, GUI.wnd.h, 0, x, y)
		GUI.redraw_z(0)
	end		
