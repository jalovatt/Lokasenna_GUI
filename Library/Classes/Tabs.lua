...
function GUI.Tabs:new(name, z, x, y, tab_w, tab_h, opts, pad)
	...
	Tab.w, Tab.h = (Tab.tab_w + Tab.pad) * #Tab.opt_array + 2*Tab.pad + 12, Tab.tab_h
    
    if Tab.full_width == nil then
        Tab.full_width = true
    end
...
function GUI.Tabs:draw()
    ...
    self.w = self.full_width and (GUI.wnd.cur_w - self.x) or math.max(self.w, (tab_w + pad) * #self.opt_array + 2*pad + 12)  
	...
	for i = #self.opt_array, 1, -1 do
		...
		if i ~= state then
			...
			self:draw_tab(tab_x, tab_y, tab_w, tab_h, dir, font, self.col_txt, self.col_tab_b, self.opt_array[i])

		end
	
	end

	self:draw_tab(x + (state - 1) * x_adj, y, tab_w, tab_h, dir, self.font_a, self.col_txt, self.col_tab_a, self.opt_array[state])
...
function GUI.Tabs:on_resize()
    
    if self.full_width then self:redraw() end
...
function GUI.Tabs:on_mouse_down()

    -- Offset for the first tab
	local adj = 0.75*self.h

	local mouse_opt = (GUI.mouse.x - (self.x + adj)) / (#self.opt_array * (self.tab_w + self.pad))
		
	mouse_opt = GUI.clamp((math.floor(mouse_opt * #self.opt_array) + 1), 1, #self.opt_array)

	self.state = mouse_opt
