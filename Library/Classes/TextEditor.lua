...
function GUI.TextEditor:val(newval)
	...
	if newval then
		self.retval = (type(newval) == "table" and newval
                                                or self:string_to_table(newval))
...
function GUI.TextEditor:on_update()
...
function GUI.TextEditor:lost_focus()
...
function GUI.TextEditor:on_mouse_down()

	-- If over the scrollbar, or we came from :on_drag with an origin point
	-- that was over the scrollbar...
	local scroll = self:over_scroll_bar()
	if scroll then 
        
        self:set_scroll_bar(scroll)        
        
    else
    
        -- Place the caret
        self.caret = self:get_caret(GUI.mouse.x, GUI.mouse.y)
...
function GUI.TextEditor:on_double_click()
...
function GUI.TextEditor:on_drag()
	
	local scroll = self:over_scroll_bar(GUI.mouse.ox, GUI.mouse.oy)
	if scroll then 
...
function GUI.TextEditor:on_type(char, mod)
...
function GUI.TextEditor:on_wheel(inc)
...
function GUI.TextEditor:draw_caption()
...
function GUI.TextEditor:draw_text()
...
function GUI.TextEditor:draw_caret()
...
function GUI.TextEditor:draw_selection()
	...
	local coords = self:get_selection()	
	
	for i = 1, #coords do
	
		-- Make sure at least part of this line is visible
		if self:selection_visible(coords[i]) then 
...
function GUI.TextEditor:draw_scroll_bars()

	-- Do we need to be here?
	local max_w, txt_h = self:get_max_length(), self:get_wnd_length()
...
function GUI.TextEditor:get_selection_coords()
...
function GUI.TextEditor:get_selection()

    local sx, sy, ex, ey = self:get_selection_coords()
...
function GUI.TextEditor:selection_visible(coords)
...
function GUI.TextEditor:select_all()
...
function GUI.TextEditor:select_word()
...
function GUI.TextEditor:clear_selection()
...
function GUI.TextEditor:delete_selection()
...
function GUI.TextEditor:get_selected_text()
...
function GUI.TextEditor:to_clipboard(cut)
...
function GUI.TextEditor:from_clipboard()
...
function GUI.TextEditor:get_max_length()
...
function GUI.TextEditor:get_wnd_length()
...
-- If not, returns nil
function GUI.TextEditor:adjust_to_window(coords)
...
function GUI.TextEditor:window_to_caret()
	...
	local adj = (	(self.caret.y < self.wnd_pos.y) and -1	)
			...
			or	(	(bot > self:get_wnd_length() and -(bot - self:get_wnd_length() - 1) ) )
...
function GUI.TextEditor:get_caret(x, y)
...
function GUI.TextEditor:over_scroll_bar(x, y)
	
	if	self:get_wnd_length() > self.wnd_h 
	and (x or GUI.mouse.x) >= (self.x + self.w - 12) then
		
		return "v"
		
	elseif 	self:get_max_length() > self.wnd_w
...
function GUI.TextEditor:set_scroll_bar(scroll)
...
function GUI.TextEditor:string_to_table(str)
...
-- i.e. Paste
function GUI.TextEditor:insert_string(str, move_caret)
	...
	local tmp = self:string_to_table(str)
...
function GUI.TextEditor:insert_char(char)
...
function GUI.TextEditor:caret_to_end()
	--[[	
...
function GUI.TextEditor:sanitize_text(str)
...
function GUI.TextEditor:ctrl_char(func, ...)
...
GUI.TextEditor.keys = {
	...
	[GUI.chars.LEFT] = function(self)
		...
		if self.caret.x < 1 and self.caret.y > 1 then
			...
			self.caret.x = self:caret_to_end()
		else
	...
	[GUI.chars.RIGHT] = function(self)
		...
		if self.caret.x == self:caret_to_end() and self.caret.y < self:get_wnd_length() then
			self.caret.y = self.caret.y + 1
			self.caret.x = 0
		else
			self.caret.x = math.min(self.caret.x + 1, self:caret_to_end() )
		end
	...
	[GUI.chars.UP] = function(self)
		...
		if self.caret.y == 1 then
			...
			self.caret.x = math.min(self.caret.x, self:caret_to_end() )
		end
	...
	[GUI.chars.DOWN] = function(self)
		...
		if self.caret.y == self:get_wnd_length() then
	...
	[GUI.chars.PGDN] = function(self)
		...
		self.wnd_pos.y = GUI.clamp(1, self:get_wnd_length() - self.wnd_h + 1, self.wnd_pos.y + self.wnd_h)
	...
	[GUI.chars.DELETE] = function(self)
		...
		elseif self.caret.x < self:caret_to_end() then
		
			local str = self.retval[self.caret.y] or ""
			self.retval[self.caret.y] = str:sub(1, self.caret.x) .. 
                                        str:sub(self.caret.x + 2)
		
		elseif self.caret.y < self:get_wnd_length() then
	...
	[GUI.chars.RETURN] = function(self)
		...
		if sel_s then self:delete_selection() end
...
function GUI.TextEditor:store_undo_state()
...
function GUI.TextEditor:get_editor_state()
