...
function GUI.Knob:new(name, z, x, y, w, caption, min, max, default, inc, vals)
    ...
    function Knob:format_retval(val)
	...
	Knob.step_angle = (3 / 2) / Knob.steps
	
	Knob.default = Knob.default or default
    Knob.cur_step = Knob.default
    
	Knob.cur_val = Knob.cur_step / Knob.steps    
	
    Knob.retval = Knob:format_retval(
                ((Knob.max - Knob.min) / Knob.steps) * Knob.cur_step + Knob.min
                                    )
...
function GUI.Knob:init()
	...
	local Ax, Ay = GUI.polar_to_cart(cur_angle, rp, o, o)
    local Bx, By = GUI.polar_to_cart(cur_angle + side_angle, r - 1, o, o)
	local Cx, Cy = GUI.polar_to_cart(cur_angle - side_angle, r - 1, o, o)
		
	-- Head
	GUI.set_color(self.col_head)
	GUI.triangle(true, Ax, Ay, Bx, By, Cx, Cy)
	GUI.set_color("elm_outline")
	GUI.triangle(false, Ax, Ay, Bx, By, Cx, Cy)	
	
	-- Body
	GUI.set_color(self.col_body)
	gfx.circle(o, o, r, 1)
	GUI.set_color("elm_outline")
	gfx.circle(o, o, r, 0)		
...
function GUI.Knob:on_delete()
	
	GUI.free_buffer(self.buff)	
...
function GUI.Knob:draw()
	...
	for i = 1, GUI.shadow_dist do
		...
		gfx.blit(   self.buff, 1, cur_angle * GUI.pi, 
...
function GUI.Knob:on_drag()
    ...
    self:set_cur_val( GUI.clamp(self.cur_val + ((ly - y) / adj), 0, 1) )
    
    --[[
	self.cur_val = self.cur_val + ((ly - y) / adj)
	if self.cur_val > 1 then self.cur_val = 1 end
	if self.cur_val < 0 then self.cur_val = 0 end
	
    
    
	self.cur_step = GUI.round(self.cur_val * self.steps)
	
	self.retval = GUI.round(((self.max - self.min) / self.steps) * self.cur_step + self.min)
    ]]--
	self:redraw()
...
-- Knob - Doubleclick
function GUI.Knob:on_double_click()
	--[[
	self.cur_step = self.default
	self.cur_val = self.cur_step / self.steps
	self.retval = GUI.round(((self.max - self.min) / self.steps) * self.cur_step + self.min)
	]]--
    
    self:set_cur_step(self.default)
...
function GUI.Knob:on_wheel()
    ...
    self:set_cur_val( GUI.clamp( self.cur_val + (GUI.mouse.inc * adj / self.steps), 0, 1))
...
function GUI.Knob:draw_caption(o, r)
    
    local str = self.caption
    
	GUI.set_font(self.font_a)
	local cx, cy = GUI.polar_to_cart(1/2, r * 2, o.x, o.y)
...
function GUI.Knob:draw_vals(o, r)        
        
    for i = 0, self.steps do
        
        local angle = (-5 / 4 ) + (i * self.step_angle)
        
        -- Highlight the current value
        if i == self.cur_step then
            GUI.set_color(self.col_head)
            GUI.set_font({GUI.fonts[self.font_b][1], GUI.fonts[self.font_b][2] * 1.2, "b"})
        else
        ...
        local output = self:format_retval( i * self.inc + self.min )
        ...
        if output ~= "" then
            ...
            local cx, cy = GUI.polar_to_cart(angle, r * 2, o.x, o.y)		
...
function GUI.Knob:set_cur_step(step)
...
function GUI.Knob:set_cur_val(val)
    
    self.cur_val = val
    self.cur_step = GUI.round(val * self.steps)
    self:set_retval()
    
end


function GUI.Knob:set_retval()

    self.retval = self:format_retval(self.inc * self.cur_step + self.min)
