...
function GUI.Slider:new(name, z, x, y, w, caption, min, max, defaults, inc, dir)
    ...
    function Slider:format_retval(val)
    ...
    function Slider:init_handles()
        ...
        for i = 1, #self.defaults do
            ...
            self.handles[i].cur_step = step
            self.handles[i].cur_val = step / self.steps
            self.handles[i].retval = self:format_retval( ((self.max - self.min) / self.steps) 
                                                        * step + self.min)
...
function GUI.Slider:init()
	...
	self.buffs = self.buffs or GUI.get_buffer(2)
...
function GUI.Slider:draw()
    ...
    local min, max = self:update_handle_coords(x, handle_w, handle_y, inc)
    
    self:draw_fill(x, y, h, min, max, inc, horz)

    self:draw_sliders(x, y, h, handle_w, handle_h, horz)
...
function GUI.Slider:val(newvals)
	...
	if newvals then
		...
		for i = 1, #self.handles do
			--[[
			table.insert(ret, (self.dir ~= "v" 	and (self.handles[i].cur_step + self.min)
												or	(self.steps - self.handles[i].cur_step)))
			]]--
...
function GUI.Slider:on_mouse_down()
    ...
    self.cur_handle = self:get_nearest_handle(mouse_val)
	
	self:set_cur_val(self.cur_handle, GUI.clamp(mouse_val, 0, 1) )
...
function GUI.Slider:on_drag()
    ...
    self:set_cur_val(cur, GUI.clamp( self.handles[cur].cur_val + ((n - ln) / adj) , 0, 1 ) )
...
function GUI.Slider:on_wheel()
    ...
    local cur = self:get_nearest_handle(mouse_val)	
    ...
    self:set_cur_val(cur, GUI.clamp( self.handles[cur].cur_val + (inc * adj / self.steps) , 0, 1) )
...
function GUI.Slider:on_double_click()
	...
	if GUI.mouse.cap & 4 == 4 then
		...
		for i = 1, #self.handles do
			...
			local diff = math.abs( self.handles[i].cur_val - mouse_val )
        ...
        self:set_cur_step(small_idx, self.handles[small_idx].default)
    ...
    -- Reset all sliders
		...
		for i = 1, #self.handles do
            ...
            self:set_cur_step(i, self.handles[i].default)
...
function GUI.Slider:update_handle_coords(x, handle_w, handle_y, inc)
    
    local min, max
    
    for i = 1, #self.handles do
        
        local center = x + inc * self.handles[i].cur_step
...
function GUI.Slider:draw_fill(x, y, h, min, max, inc, horz)
    
    -- Get the color
	if (#self.handles > 1) 
    or self.handles[1].cur_step ~= self.handles[1].default then
...
function GUI.Slider:set_fill()
    ...
    if self.col_fill_a and #self.handles == 1 then
        ...
        local grad_step = self.handles[1].cur_step / self.steps
...
function GUI.Slider:draw_sliders(x, y, h, handle_w, handle_h, horz)
    ...
    for i = #self.handles, 1, -1 do
        ...
        if self.show_values then
            ...
            if horz then
                self:draw_slider_value(x, y, i)
            else
                self:draw_slider_value(y, x - 2, i)
            end
        ...
        if self.show_handles then
            ...
            if horz then
                self:draw_slider_handle(handle_x, handle_y, handle_w, handle_h)
            else
                self:draw_slider_handle(handle_y, handle_x, handle_h, handle_w)
            end
...
function GUI.Slider:draw_slider_value(x, y, sldr)
    ...
    GUI.txt_bg(output, self.bg)
    gfx.drawstr(output, 1)
    
end

                
function GUI.Slider:draw_slider_handle(hx, hy, hw, hh)
...
function GUI.Slider:draw_caption()
...
function GUI.Slider:get_nearest_handle(val)
    
	local small_diff, small_idx
	
	for i = 1, #self.handles do

		local diff = math.abs( self.handles[i].cur_val - val )
...
function GUI.Slider:set_cur_step(sldr, step)

    self.handles[sldr].cur_step = step
    self.handles[sldr].cur_val = self.handles[sldr].cur_step / self.steps
    self:set_retval(sldr)
    
    
end


function GUI.Slider:set_cur_val(sldr, val)
   
    self.handles[sldr].cur_val = val
    self.handles[sldr].cur_step = GUI.round(val * self.steps)
    self:set_retval(sldr)
    
end


function GUI.Slider:set_retval(sldr)

    local val = self.dir == "h" and self.inc * self.handles[sldr].cur_step + self.min
                                or self.min - self.inc * self.handles[sldr].cur_step
    
    self.handles[sldr].retval = self:format_retval(val)
